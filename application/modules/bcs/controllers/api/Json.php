 ...
 */
class Json extends BaseHmvcREST_Controller
{
	/**
	...
	protected function markers_geojson($incidents, $category_id, $color, $icon, $include_geometries = TRUE)
	{
		...
		foreach ($incidents as $marker) {
			...
			if (isset($marker['bldg_gen_det/latitude']) AND isset($marker['bldg_gen_det/longitude'])) {
				$latitude = $marker['bldg_gen_det/latitude'];
				$longitude = $marker['bldg_gen_det/longitude'];
			}else {
			...
			$item_name = $marker['ho_detail/ho_name'];'';//$this->get_title($marker->incident_title, $link);
	...
	protected function clusters_geojson($incidents, $category_id, $color, $icon)
	{
		...
		while (count($markers)) {
			...
			$marker['id']=$marker['_uuid'];
			if (isset($marker['bldg_gen_det/latitude']) AND isset($marker['bldg_gen_det/longitude'])) {
				$marker_latitude =$marker['bldg_gen_det/latitude'];
				$marker_longitude = $marker['bldg_gen_det/longitude'];
			} else {
				// No location - skip this report
				continue;
			}
			...
			foreach ($markers as $key => $target) {
				// Handle both reports::fetch_incidents() response and actual ORM objects
				if (isset($target['bldg_gen_det/latitude']) AND isset($target['bldg_gen_det/longitude'])) {
					$target_latitude = $target['bldg_gen_det/latitude'];
					$target_longitude = $target['bldg_gen_det/longitude'];
				}  else {
					// No location - skip this report
					continue;
				}
